...
local function characterCreation(ply)
    ...
    armyBtn:SetImage(MRP.Factions[1]["flag"])
    ...
    rebelBtn:SetImage(MRP.Factions[2]["flag"])
    ...
    local function chooseRegiment()
        ...
        for i = 1, #MRP.Factions[faction] do
            local reg = vgui.Create("DImageButton")
            reg:SetImage(MRP.Factions[faction][i]["insignia"])
            reg:SizeToContents()
...
local function characterSelection(ply)
    ...
    function loadCharacter()
        ...
        local regImg = MRP.Factions[faction][regiment]
        ...
        rankPanel:SetImage(MRP.Factions[faction][regiment][rank]["shoulderrank"])
        ...
        selecPanel.pmodel.Entity.MRPGetID = function(_, MRPCategory)
