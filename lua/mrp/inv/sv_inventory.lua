...
hook.Add('simfphysPhysicsCollide', 'MRPsimfphysPhysicsCollide', function(ent, data, _)
    ...
    local mrpid = MRP.ClassID[item.ClassName]
    if mrpid and item ~= lastent then
        for k = 1, 20 do
...
function MRP.PickupAmmoBox(ply, ent)
    ...
    for k = 1, 20 do
        ...
        local slotContainsSameAmmo = ply:GetNWInt("Inventory" .. k) == mrpid
        local slotAmmo = ply:GetNWInt("Inventory" .. k .. "Rounds")
        local boxCap = MRP.EntityTable(mrpid).Capacity
        ...
        if ply:GetNWInt("Inventory" .. k) == 1 and ent.Rounds > 0 then
            ply:GiveAmmo(ent.Rounds, ent.Ammo)
            ply:SetNWInt("Inventory" .. k, mrpid)
...
function MRP.PickupRucksack(ply, ent)
    local mrpid = MRP.ClassID[ent.ClassName]
    ply:SetNWInt("Rucksack", mrpid)
    sql.Query(
        "UPDATE " .. tbName .. " SET Rucksack = " .. mrpid ..
        " WHERE CharacterID = " .. ply:MRPCharacterID() .. ";"
    )
...
function MRP.PickupWep(ply, ent)
    ...
    Log.d("PickupWep ", mrpid)
    wep:SetClip1(ent.Rounds)
    ply:SetNWInt(ent.MRPCategory, mrpid)
