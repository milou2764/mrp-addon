...
local function rankChange(admin, targetId, inc)
    ...
    if canPromote(admin, target) then
        ...
        local limitUp = #MRP.Factions[faction][regiment]
        ...
        local bodyId = MRP.Factions[faction][regiment][target:GetNWInt("Rank")]["bodygroupVal"]
        ...
        target:setRPName(target:MRPRankShort().." - "..target:MRPName())
    else
